...
class Ba_importer extends Module
{
    ...
    public function addFeatures($id_product, $id_lang, $id_feature, $feature_value, $customize = 0)
    {
        ...
        $sql = 'SELECT a.id_feature_value FROM ' . _DB_PREFIX_ . 'feature_value AS a INNER JOIN '
                ...
                . " WHERE a.id_feature=$id_feature AND b.value='" .$feature_value. "'";
    ...
    public function addFeaturesLang($id_product, $id_lang, $feature_column, $row, $customize = 0)
    {
        ...
        foreach ($a as $key => $row2) {
            ...
            foreach ($a[$key] as $key2 => $row3) {
                ...
                $id_feature_value2 = (int) Db::getInstance()->getValue($sql2, false);
            ...
            if ($check_feature_exist == 'tontaicocustom') {
                if ($customize == 1) {
                    $sql = "UPDATE " . _DB_PREFIX_ . "feature_value SET custom = 0 WHERE id_feature_value = $d";
                    Db::getInstance()->query($sql);
                }
                ...
                if (!empty($id_attachment)) {
                    ...
                    if ($_post["multi_lang"] == "0") {
                        ...
                        foreach ($arr_lang as $key_arr_lang => $value_arr_lang) {
                            ...
                            $attachment->name[$key_arr_lang] = $displayname;
                ...
                foreach ($languages as $lang) {
                    ...
                    if (is_array($categories) && count($categories)) {
                        ...
                        foreach ($categories as $category_name) {
                            if ($category_name == '') {
                                $id_root_category = (int) Configuration::get('PS_HOME_CATEGORY');
