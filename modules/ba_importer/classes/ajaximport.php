        ...
        foreach ($array as $key => $row) {
            if ($key >= $start && $key <= $product_end) {
                ...
                if ($_post["identify_existing_items"] == "Reference code") {
                    $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'product WHERE reference = "' . pSQL(@$_reference) . '"';
                    if ($results = Db::getInstance()->ExecuteS($sql, true, false)) {
                ...
                if ($_post["identify_existing_items"] == "Product ID") {
                    $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'product WHERE id_product = "' . (int) $_product_id . '"';
                    if ($results = Db::getInstance()->ExecuteS($sql, true, false)) {
                ...
                if ($_post["identify_existing_items"] == "EAN-13 or JAN barcode") {
                    $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'product WHERE ean13 = "' . pSQL($_ean13) . '"';
                    if ($results = Db::getInstance()->ExecuteS($sql, true, false)) {
                ...
                if ($_post["identify_existing_items"] == "UPC barcode") {
                    $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'product WHERE upc = "' . pSQL($_upc) . '"';
                    if ($results = Db::getInstance()->ExecuteS($sql, true, false)) {
                ...
                // since 1.1.2
								// MOD 095.1: Replace the "Combination Reference" conditional for matching with a product.
								$matched_with_combination = false;

								if (
									   $_post["identify_existing_items"] == "Combination Reference"
									|| (
										   empty( $id_row )
										&& $_post["identify_existing_items"] == "Reference code"
									)
								) {
                        ...
                        if (!empty($results)) {
                            foreach ($results as $p_row2) {
                                $id_row[] = $p_row2['id_product'];
														...
														$matched_with_combination = true;

														// "Force" combination values if they do not exist by assigning them existing [parent product] columns.
														if (
															   $_post["identify_existing_items"] == "Reference code"
															&& !isset( $array_colum['combination_field']['ba_combination_reference'] )
															&& isset( $array_colum['Reference'] )
															&& isset ( $array_colum['Quantities'] )
														) {
															$array_combination_field = [
																'ba_combination_reference' => $array_colum['Reference'],
																'ba_combination_quantity'  => $array_colum['Quantities'],
																'combination_images'       => []
															];
															$_combination_quantity = $row[$array_colum['Quantities']];
															$_quantities = null;
														}
                        }
                    }
                ...
                if (!empty($id_row)) {
                    if ($_post["existing_items"] == "Update") {
                        foreach ($id_row as $row_id) {
                            ...
                            $io = (int) StockAvailable::getStockAvailableIdByProductId($id_product, 0, $is);
														// MOD 087.3: Modify method of updating quantity.
														if (!$io) {
                                ...
                                $where_s = 'id_stock_available = '.$io;
																...
																$stock_available = new StockAvailable( $io );
																$quantity_old = ( int )$stock_available->quantity;

                                // $db->update('stock_available', $data_c, $where_s);
                            }

														if ( isset( $data_c['quantity'] ) ) {
															// Fire PS hook & default behaviour here.
															StockAvailable::setQuantity( $data_c['id_product'], $data_c['id_product_attribute'], $data_c['quantity'] );

															$geewiz_supplier_ids = [10, 77, 118, 66, 78, 79, 76, 121, 122];
															$product_id_supplier = (
																!empty( $id_supplier )
																	? ( int )end( $id_supplier )
																	: $db->getValue( 'SELECT id_supplier FROM ' . _DB_PREFIX_ . 'product WHERE id_product = ' . $data_c['id_product'] )
															);
															$cookie              = new Cookie( 'psAdmin' );
															$employee            = new Employee( $cookie->id_employee );
															$deltaQuantity       = ( isset( $quantity_old ) ? -1 * ( ( int )$quantity_old - ( int )$data_c['quantity'] ) : ( int )$data_c['quantity'] );
															$is_unsigned_int     = ( Validate::isUnsignedInt( $deltaQuantity ) ? true : false );

															// Manually insert a new movement entry here.
															// Adding a movement entry via "StockAvailable::setQuantity(...)" does not appear to succeed
															// when done via an AJAX call (to a custom PHP file).
															if (
																   0 !== abs( $deltaQuantity )
																&& in_array( $product_id_supplier, $geewiz_supplier_ids )
															) {
																$data_insert = [
																	'id_stock'            => $io,
																	'id_stock_mvt_reason' => ( $is_unsigned_int ? 13 : 14 ),
																	'id_employee'         => $employee->id,
																	'employee_lastname'   => $employee->lastname,
																	'employee_firstname'  => $employee->firstname,
																	'physical_quantity'   => abs( $deltaQuantity ),
																	'date_add'            => date( 'Y-m-d H:i:s' ),
																	'sign'                => ( $is_unsigned_int ? 1 : -1 )
																];

																$db->insert( 'stock_mvt', $data_insert );
															}
														}
														// MOD 087.3 END
                            ...
                            if (@$_wholesale_price !== null) {
                                $data['wholesale_price'] = @(float) ($_wholesale_price);
														...
														// MOD 095.2: Modify matched product "reference" conditional - do not update if there is a matched combination.
														if (
															   @$_reference != null
															&& !$matched_with_combination
														) {
                            ...
                            if (@$_out_of_stock !== null) {
                                $data['out_of_stock'] = (int) $_out_of_stock;
														...
														// MOD 095.3: Modify matched product "quantities" conditional - do not update if there is a matched combination.
														if (
															   $_quantities !== null
															&& !$matched_with_combination
														) {
                            ...
                            if ($_post["identify_existing_items_combi"] == 'Attributes') {
                                if (!empty($array_attribute)) {
                                    ...
                                    if (!empty($_combination_quantity)) {
                    ...
                    if ($_post["new_items"] == "Add") {
                        ...
                        $is = $data['id_shop'];
                        $io = (int) StockAvailable::getStockAvailableIdByProductId($id_product, 0, $is);
												// MOD 087.4: Modify method of updating quantity.
												if (!$io) {
                            $shop_group = new ShopGroup((int)Shop::getGroupFromShop((int)$is));
                            // if quantities are shared between shops of the group
                            if ($shop_group->share_stock) {
                            ...
                            // $db->update('stock_available', $data, $where_s);
												...
												if ( isset( $data['quantity'] ) ) {
													// Fire PS hook & default behaviour here.
													StockAvailable::setQuantity( $data['id_product'], $data['id_product_attribute'], $data['quantity'] );

													$geewiz_supplier_ids = [10, 77, 118, 66, 78, 79, 76, 121, 122];
													$product_id_supplier = (
														!empty( $id_supplier )
															? ( int )end( $id_supplier )
															: $db->getValue( 'SELECT id_supplier FROM ' . _DB_PREFIX_ . 'product WHERE id_product = ' . $data['id_product'] )
													);
													$cookie              = new Cookie( 'psAdmin' );
													$employee            = new Employee( $cookie->id_employee );
                        ...
                        $this->array_result[] = '';
												// MOD 087.8: Capture "ignored" items in an array.
												if (
													   'IgnoreAndEmail' === $_post["new_items"]
													|| 'IgnoreAndEmail' === $_post["existing_items"]
												) {
													$email_rows[$key] = [
				...
				// MOD 087.9: Prepare & email "ignored" items.
				if ( !empty( $email_rows ) ) {
					...
					$email_rows_html = $this->fetch( 'module:ba_importer/views/templates/admin/partials/emailignoredrows.tpl' );

					$date      = new DateTime( 'now' );
					$subject   = '"ba_importer" ignored rows - ' . $date->format( 'Y-m-d H:i:s' );
					$mail_vars = array(
						'{date_add}'   => $date->format( 'Y-m-d H:i:s' ),
						'{shop_name}'  => $configuration['PS_SHOP_NAME'],
						'{shop_url}'   => Context::getContext()->link->getPageLink( 'index' ),
						'{shop_logo}'  => Context::getContext()->link->getPageLink( 'index' ) . 'img/' . $configuration['PS_LOGO_MAIL'],
						'{email_rows}' => $email_rows_html
					);

					Mail::Send(
						( int )( $configuration['PS_LANG_DEFAULT'] ),           // defaut language id
						'ignoredrows',                                          // email template file to be use
						$subject,                                               // email subject
						$mail_vars,                                             // mail vars
						$configuration['PS_SHOP_EMAIL'],                        // receiver email address
						NULL,                                                   // receiver name
						$configuration['PS_SHOP_EMAIL'],                        // from email address
