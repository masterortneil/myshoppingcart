        ...
        foreach ($array as $key => $row) {
            if ($key >= $start && $key <= $product_end) {
                ...
                if ($_post["identify_existing_items"] == "Reference code") {
                    ...
                    $sql .= 'WHERE reference = "' . pSQL(@$_reference) . '"';
                    if ($results = Db::getInstance()->ExecuteS($sql, true, false)) {
                ...
                if ($_post["identify_existing_items"] == "Product ID") {
                    $sql = 'SELECT * FROM ' . _DB_PREFIX_ . 'product ';
                    $sql .= 'WHERE id_product = "' . (int) $_product_id . '"';
                    if ($results = Db::getInstance()->ExecuteS($sql, true, false)) {
                ...
                if (!empty($id_row)) {
                    if ($_post["existing_items"] == "Update") {
                        ...
                        foreach ($id_row as $row_id) {
                            ...
                            $io = (int) StockAvailable::getStockAvailableIdByProductId($id_product, 0, $is);
														// MOD 087.1: Modify method of updating quantity.
                                ...
                                if (@$array_colum["category_associated"]['0'] != null) {
                                    ...
                                    foreach (@$array_colum["category_associated"] as $value) {
                                        ...
                                        if (isset($array_colum["main_category"])) {
                                            ...
                                            $vca = str_replace($characters_category, '/', $value_row); //value_category_associated
                                            if (!empty($vca)) {
                                                $il = $id_lang;
                                                $ish = $this->shop_id;
                                                if ($_post['cate_exist'] == '0') {
                    ...
                    if ($_post["new_items"] == "Add") {
                        ...
                        $is = $data['id_shop'];
                        $io = (int) StockAvailable::getStockAvailableIdByProductId($id_product, 0, $is);
												// MOD 087.2: Modify method of updating quantity.
												StockAvailable::setQuantity( $data['id_product'], $data['id_product_attribute'], $data['quantity'] );
												// MOD 087.2 END
                        $arr_log_import = 'finished' . "\n";
                        $this->logImport($arr_log_import, 'auto', $this->get_id_config);
                        ...
                        $this->array_result[] = '';
												// MOD 087.10: Capture "ignored" items in an array.
												if (
				...
				// MOD 087.11: Prepare & email "ignored" items.
				if ( !empty( $email_rows ) ) {
